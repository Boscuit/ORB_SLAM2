...
int ORBmatcher::SearchForInitialization(LoadedKeyFrame* pLKF, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
    ...
    for(size_t i1=0, iend1=pLKF->mvKeysUn.size(); i1<iend1; i1++)
    {
        ...
        cv::Mat d1 = pLKF->mDescriptors.row(i1);
...
int ORBmatcher::SearchByBoW(LoadedKeyFrame* pLKF,Frame &F, vector<int> &vnMatches21)
{
    ...
    const DBoW2::FeatureVector &vFeatVecKF = pLKF->mFeatVec;
    ...
    while(KFit != KFend && Fit != Fend)
    {
        if(KFit->first == Fit->first)
        {
            ...
            for(int iKF=0; iKF<vIndicesKF.size(); iKF++)
            {
                const unsigned int realIdxKF = vIndicesKF[iKF];
                ...
                const cv::Mat &dLKF= pLKF->mDescriptors.row(realIdxKF);
                ...
                for(int iF=0; iF<vIndicesF.size(); iF++)
                {
                    const unsigned int realIdxF = vIndicesF[iF];
